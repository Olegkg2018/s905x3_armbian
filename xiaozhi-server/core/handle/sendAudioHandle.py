...
async def send_tts_message(conn: "ConnectionHandler", state, text=None):
    ...
    if text is not None:
        ds.add_chat("assistant", text)
    if state == "start":
        ds.set_state("speaking")
        ds.add_event("TTS: Начало озвучивания ответа")
    elif state == "stop":
        ds.set_state("idle")
        ds.add_event("TTS: Озвучивание завершено")
...
async def send_stt_message(conn: "ConnectionHandler", text):
    ...
    from core.utils import dashboard_shared as ds
    ds.add_chat("user", stt_text)
    ds.set_state("thinking")
    ds.add_event(f"ASR: Распознан текст: {stt_text}")
