...
class ListenTextMessageHandler(TextMessageHandler):
    ...
    async def handle(self, conn: "ConnectionHandler", msg_json: Dict[str, Any]) -> None:
        ...
        if msg_json["state"] == "start":
            # ПРАВКА: раньше в manual/push-to-talk режиме нажатие кнопки во
            # время воспроизведения ответа/музыки НИЧЕГО не прерывало —
            # reset_audio_states() только чистил буферы записи, а сам обрыв
            # (handleAbortMessage) вызывался лишь позже, из startToChat(),
            # причём для manual-режима был явно пропущен (см. receiveAudioHandle.py).
            # В результате старое воспроизведение доигрывало до конца, и только
            # потом начинался ответ на новую команду — пользователь физически
            # не мог "перебить" колонку кнопкой.
            # Для push-to-talk это не требует AEC/полного дуплекса: пока кнопка
            # не нажата, микрофон всё равно не слушает, поэтому мгновенный обрыв
            # по нажатию кнопки полностью безопасен и не вызывает эха.
